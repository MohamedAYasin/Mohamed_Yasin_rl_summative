...
from environment.custom_env import EducationHubEnv
from environment.rendering import render_school_env
...
def simulate_trained_model(model_path):
    ...
    pygame.display.set_caption("Trained DQN School Simulation")
    
    # Load environment and DQN model
    env = EducationHubEnv(grid_size=5, max_steps=100, render_mode='human')  # Update to your custom env
    ...
    while running:
        ...
        # Render the environment state (using your customized rendering)
...
if __name__ == '__main__':
    # path to DQN model
    simulate_trained_model("models/dqn/dqn_final_model.zip")  # Update to the correct model path
